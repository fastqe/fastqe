...
class FastqStats(object):
    ...
    def __repr__(self):
        ...
        return (
            "FastqStats(num_seqs={}, num_bases={}, min_len={}, max_len={}, "
            "average={})".format(
    ...
    def from_file(self, fastq_file, read_size, minlen_threshold=DEFAULT_MIN_LEN):
        ...
        mins = np.full(
            read_size, np.inf
        )  # set minimum to inf so any value observed is smaller
        ...
        means_fp = (
            cleaned / counts_cleaned
        )  # use counts to fix long read where not all seq same length
        fake_seq = "".join(["a"] * len(means_fp.round()))

        record_mean = SeqRecord(
            Seq(fake_seq),
            id="test",
            name="mean scores",
    ...
    if options.fastq_files:
        for fastq_filename in options.fastq_files:
            ...
            try:
                ...
                with fastq_file:
                    stats = FastqStats().from_file(
                        fastq_file, read_size, options.minlen
                    )
                    print_output(
                        stats,
                        fastq_filename,
...
def print_output(
    ...
    if output_type == CASE_HTML or output_type == CASE_HTML_ESCAPE:
        ...
        mean_windowed = np.nanmean(
            np.r_[
                stats_object.quality_scores_mean.letter_annotations["phred_quality"],
                np.nan + np.zeros((-(seq_len - 1) % window,)),
            ].reshape(-1, window),
            axis=-1,
        )
        min_windowed = np.nanmean(
            np.r_[
        ...
        max_windowed = np.nanmean(
            np.r_[
                stats_object.quality_scores_maxs.letter_annotations["phred_quality"],
                np.nan + np.zeros((-(seq_len - 1) % window,)),
            ].reshape(-1, window),
            axis=-1,
        )

        interleaved_scores = np.dstack(
            (max_windowed, mean_windowed, min_windowed, mean_windowed)
        ).flatten()
        ...
        interleaved_scores_seq = SeqRecord(
            Seq(fake_seq_interleaved),
            id="test",
            name="mean scores",
            description="example with mean fastq socres",
        ...
        for ind, (max_e, mean_e, min_e, mean_e_2) in enumerate(just_emojis_4):
            ...
            if window == 1:
                ...
                paging_text = "-" + str(
                    min(window_range[ind] + window - 1, seq_len - 1)
                )
            html_text += html_format_fstring.format(
                str(window_range[ind]) + paging_text, *print_values
            )
        ...
        print(
            html_header
            + "️▶️  "
            + html_format_fstring_header.format(
                stats_object.num_seqs,
                stats_object.max_len,
                stats_object.min_len,
                stats_object.average,
                stats_object.num_bases,
            )
            + "   "
            + stats_object.pretty(fastq_filename)
            + "<br>"
            + "".join(html_text)
            + "<br>"
            + html_footer,
        ...
        if output_type == CASE_BOTH or output_type == CASE_MAX:
            print(
                ...
                "max" + mapping_text,
                map_scores(
                    stats_object.quality_scores_maxs,
                    mapping_dict=mapping_dict,
                    default_value=mapping_default,
                    spacer=spacer,
                ),
        ...
        print(
            ...
            "mean" + mapping_text,
            map_scores(
                stats_object.quality_scores_mean,
                mapping_dict=mapping_dict,
                default_value=mapping_default,
                spacer=spacer,
            ),
        ...
        if output_type == CASE_BOTH or output_type == CASE_MIN:
            print(
                ...
                "min" + mapping_text,
                map_scores(
                    stats_object.quality_scores_mins,
                    mapping_dict=mapping_dict,
                    default_value=mapping_default,
                    spacer=spacer,
                ),
